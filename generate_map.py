...
def add_attractions_and_queues(
    grid, nb_attraction=20, queue_length=4, min_dist=5
):
    height = len(grid)
    width_grid = len(grid[0])
    attractions = []
    ...
    while len(attractions) < nb_attraction and attempts < max_attempts:
        ...
        if not any(grid[ny][nx] == ROAD for ny, nx in neighbors):
            attempts += 1
            continue
        ...
        for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            for i in range(1, queue_length + 1):
                ny, nx = y + dy * i, x + dx * i
                if ny < 0 or ny >= height or nx < 0 or nx >= width_grid:
                    valid = False
                    break
                if grid[ny][nx] != EMPTY:
                    valid = False
        ...
        for x in range(1, width_grid - 1):
            if grid[y][x] == EMPTY:
                ...
                for d1, d2, o1, o2, o3, o4 in diag_pairs:
                    if (
                        0 <= d1[0] < height
                        and 0 <= d1[1] < width_grid
                        and 0 <= d2[0] < height
                        and 0 <= d2[1] < width_grid
                        and grid[d1[0]][d1[1]] == ROAD
                        and grid[d2[0]][d2[1]] == ROAD
                        and grid[o1[0]][o1[1]] == EMPTY
                        and grid[o2[0]][o2[1]] == EMPTY
                    ):
                        grid[y][x] = ROAD
                        additions += 1
                        break
...
def export_to_txt(grid, filename):
    with open(filename, "w") as f:
        for row in grid:
            f.write("".join(row) + "\n")


def generate_theme_park(
    width=150,
    height=50,
    entries=3,
    nodes=400,
